...
public class DeviceLoggingService
{
    ...
    public void HandleReading(DeviceReading r)
    {
        ...
        {
            _logger.Danger(Guid.Parse("11111111-1111-1111-1111-111111111111"), $": [DVID]:{r.DeviceId} - [MSSG]:{r.DeviceType} VERY low: {r.Value:F2} {r.Unit}");
            return;
        }

        if (t.Low is double low && r.Value < low)
        {
            _logger.Warning(Guid.Parse("11111111-1111-1111-1111-111111111111"), $": [DVID]:{r.DeviceId} - [MSSG]:{r.DeviceType} low: {r.Value:F2} {r.Unit}");
            return;
        }

        if (t.High is double high && r.Value > high)
        {
            _logger.Danger(Guid.Parse("11111111-1111-1111-1111-111111111111"), $": [DVID]:{r.DeviceId} - [MSSG]:{r.DeviceType} high: {r.Value:F2} {r.Unit}");
            return;
        }

        if (t.VeryHigh is double vHigh && r.Value > vHigh)
        {
            _logger.Warning(Guid.Parse("11111111-1111-1111-1111-111111111111"), $": [DVID]:{r.DeviceId} - [MSSG]:{r.DeviceType} VERY high: {r.Value:F2} {r.Unit}");
            return;
        }
    }
}